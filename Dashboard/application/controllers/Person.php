...
// load tables
	...
	{
		$list = $this->person->get_datatables_tab_admin_pos();
		$data = array();
		$no = $_POST['start'];
		foreach ($list as $person) {
			$no++;
			...
			if($id==0){
			//add html for action
			$row[] = '<a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="history_user('."'".$person->POS_IMEI."'".')"><i class="glyphicon glyphicon-list-alt"></i> Historial</a>';
	
			}
			elseif (strlen($id)==3) {
				# code...
			if($person->ESTADO==0){
			$row[] = '<a class="btn btn-success" href="javascript:void(0)" title="Activo" onclick="delete_device('."'".$person->POS_IMEI."'".')"><i class="glyphicon glyphicon-ok-sign"></i> </a>
					  <a class="btn btn-sm btn-primary" href="javascript:void(0)" title="Edit" onclick="password_device('."'".$person->POS_IMEI."'".')"><i class="glyphicon glyphicon-lock"></i> </a>';
			}
			else{
			$row[] = '<a class="btn btn-danger" href="javascript:void(0)" title="Desactivado"><i class="glyphicon glyphicon-ok-sign"></i> </a>';
